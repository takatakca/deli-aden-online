...
function createInventory({ dbApi, realtime, sms, logOrderEvent, sendMail }) {
  ...
  async function checkLowStock(ing) {
    ...
    if (LOW_SMS && sms && typeof sms.notifyAdmin === "function") {
      try { await sms.notifyAdmin(msg, "admin_low_stock"); } catch (e) { console.error("[inventory] low-stock sms", e.message); }
    }
    if (LOW_EMAIL && typeof sendMail === "function") {
      try { await sendMail({ subject: "Stock bas — " + ing.name, text: msg }); }
